...
length 2. The strings $u_1u_2$ and $v_1v_2$ can only be confused if one of the 
...
In graph-theoretic terms this amounts to considering the \textit{product} $G_1 \times G_2$ 
...
connected by an edge if and only if $u_i = v_i$ or $u_iv_i \in E_i$ for $i = 1,2$. The 
...
be a largest independent set in $G$, $|U| = \alpha$. The $\alpha^n$ vertices in $G^n$ of the 
...
By encoding symbols into longer strings we can make error-free communication more efficient.\\
...
\marginnote{\includegraphics[width=\marginparwidth-20pt]{images/C5xC5.png}\vspace*{-10pt}\ \ \ \center The graph $C_5 \times C_5$}
...
the $5$-cycle as depicted earlier, or at the product $C_5 \times C_5$ as drawn on the 
...
of size $3$, and so on. Let $\mathcal{C}$ be the set of cliques in $G$. Consider an arbitrary 
...
an \textit{orthonormal representation} of $G$. Clearly, such a representation always 
...
% ADD IMAGE IN THE LEFT MARGIN
\marginnote{\hspace*{-1cm}\includegraphics[width=\marginparwidth-30pt]{images/umbrella.png}\vspace*{-8pt}\\\center\hspace*{-40pt}\footnotesize{The Lov\'asz umbrella}}[-2cm]

\begin{equation}
  \Theta(C_5) \leq \frac{1}{h^2}. \label{four}
\end{equation}

A simple calculation yields $h^2 = \frac{1}{\sqrt{5}}$; see the box on the next page. From   
...
\begin{equation*}
  \langle x,y \rangle = x_1y_1 + \dotsb + x_sy_s
\end{equation*}
